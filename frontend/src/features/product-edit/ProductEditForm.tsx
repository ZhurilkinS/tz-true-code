...
// Типы для входных данных формы (то что приходит из полей ввода)
...
export const ProductEditForm = ({
  ...
  const onSubmit = async (data: ProductFormOutput) => {
    try {
      ...
      // Правильно обрабатываем изображение - используем то же имя, что ожидает бэкенд
      console.log("Image data:", data.image);
      if (data.image instanceof File) {
        formData.append("image", data.image); // Используем "image" вместо "file"
      ...
      await productsServices.updateProduct(product.id, formData);
      onSuccess();
      onHide();
    } catch (err) {
